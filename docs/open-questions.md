...
| 15 | §5.3's row-count checksum has no declared row total to check against | §5.3 asks that `N` equal "the page footer's declared count". The real export's footer is `Seite X von Y` — a **page** count; the format carries no explicit row total | **Interpreted (M2)**: enforce `Nr.` contiguous `1..N` (no gaps, no duplicates) **and** every footer-declared page `1..Y` actually parsed. This still hard-fails on the dropped-page case §5.3 names as the worst realistic failure — confirm the reading, or supply a sample whose footer does declare a row total |
